# ---------------------------------------------------------------------------
# Stack: staging-bootstrap (Layer 2)
# Instala o ArgoCD NO cluster de staging (modelo: um ArgoCD por cluster).
# Roda DEPOIS do stack de infra ("staging").
#
...
module "argocd" {
  ...
  # Path por-cluster: instala o ArgoCD + registra o root app-of-apps de staging
  # (que puxa só as Applications deste cluster).
  manifests_path = "${path.root}/../../../gitops/clusters/staging"
}
